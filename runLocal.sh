...
function refresh_config() {
	...
	# start at 8081 so an IDE can run a single service at 8080
	httpPort=8081
	debugPort=4001
	...
	for file in $files; do
		...
		if [ -n "$integrationTestsProperties" ]; then
			echo "### Autogenerated - DO NOT EDIT" > "$integrationTestsProperties"
			echo "integration.tests.endpoint=http://localhost:$httpPort/" >> "$integrationTestsProperties"
		fi
		httpPort=$((httpPort+1))
...
function integration_tests() {
	...
	for service in $integrationTestsJars; do
		...
		if [[ "$services" =~ $name ]]; then
			...
			echo -e "\e[1;33m$name:\e[0m \e[0;34mjava -jar $jar\e[0m\n"
			java -jar "$jar"
			exit_code=$?
			if [ $exit_code == 0 ]; then
